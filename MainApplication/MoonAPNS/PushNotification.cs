...
namespace MainApplication.MoonAPNS
{
	public class PushNotification
	{
		// private static readonly Logger Logger = LogManager.GetCurrentClassLogger();
		//static bool m_debuggingNotifications = false;
		...
		{
			//if (m_debuggingNotifications == true)
			//{
			//	Program.Display("Payload queue received.");
			//}
			...
			foreach (var item in queue)
			{
				...
				try
				{
						...
						//check lenght of device token, if its shorter or longer stop generating Payload.
					{
						...
						//if (m_debuggingNotifications == true)
						//{
						//	Program.Display("Notification successfully sent to APNS server for Device Toekn : " + item.DeviceToken);
						//}
					...
					else
					{
						//if (m_debuggingNotifications == true)
						//{
						//	Program.Display("Invalid device token length, possible simulator entry: " + item.DeviceToken);
						//}
				...
				catch //(Exception ex)
				{
					//if (m_debuggingNotifications == true)
					//{
					//	Program.Display("An error occurred on sending payload for device token {0} - {1}"+ item.DeviceToken+ ex.Message);
					//}
		...
		private void ReadResponse(IAsyncResult ar)
		{
			...
			try
			{
				...
				if (_apnsStream.CanRead)
				{
					...
					//if (m_debuggingNotifications == true)
					//{
					//	Program.Display("Apple rejected palyload for device token : " + _notifications[payLoadIndex].DeviceToken);
					//	Program.Display("Apple Error code : " + _errorList[status]);
					//	Program.Display("Connection terminated by Apple.");
					//}
			...
			catch // (Exception ex)
			{
				//if (m_debuggingNotifications == true)
				//{
				//	Program.Display("An error occurred while reading Apple response for token {0} - {1}"+ _notifications[payLoadIndex].DeviceToken+ ex.Message);
				//}
		...
		private void Connect(string host, int port, X509CertificateCollection certificates)
		{
			//if (m_debuggingNotifications == true)
			//{
			//	Program.Display("Connecting to apple server.");
			//}
			...
			catch //(SocketException ex)
			{
				//if (m_debuggingNotifications == true)
				//{
				//	Program.Display("An error occurred while connecting to APNS servers - " + ex.Message);
				//}
		...
		private void Disconnect()
		{
			...
			catch //(Exception ex)
			{
				//if (m_debuggingNotifications == true)
				//{
				//	Program.Display("An error occurred while disconnecting. - " + ex.Message);
				//}
		...
		private bool OpenSslStream(string host, X509CertificateCollection certificates)
		{
			//if (m_debuggingNotifications == true)
			//{
			//	Program.Display("Creating SSL connection.");
			//}
			...
			if (!_apnsStream.IsMutuallyAuthenticated)
			{
				//if (m_debuggingNotifications == true)
				//{
				//	Program.Display("SSL Stream Failed to Authenticate");
				//}
			...
			if (!_apnsStream.CanWrite)
			{
				//if (m_debuggingNotifications == true)
				//{
				//	Program.Display("SSL Stream is not Writable");
				//}
		...
		private static byte[] GeneratePayload(NotificationPayload payload)
		{
			try
			{
				...
				//if (m_debuggingNotifications == true)
				//{
				//	Program.Display("Payload generated for " + payload.DeviceToken + " : " + apnMessage);
				//}
			...
			catch //(Exception ex)
			{
				//if (m_debuggingNotifications == true)
				//{
				//	Program.Display("Unable to generate payload - " + ex.Message);
				//}
		...
		public List<Feedback> GetFeedBack()
		{
			try
			{
				...
				//if (m_debuggingNotifications == true)
				//{
				//	Program.Display("Connecting to feedback service.");
				//}
				...
				if (_conected)
				{
					...
					//if (m_debuggingNotifications == true)
					//{
					//	Program.Display("Feedback response received.");
					//}
					...
					if (recd == 0)
					{
						//if (m_debuggingNotifications == true)
						//{
						//	Program.Display("Feedback response is empty.");
						//}
					...
					while (recd > 0)
					{
						//if (m_debuggingNotifications == true)
						//{
						//	Program.Display("processing feedback response");
						//}
					...
					if (feedbacks.Count > 0)
					{
						//if (m_debuggingNotifications == true)
						//{
						//	Program.Display("Total {0} feedbacks received."+ feedbacks.Count);
						//}
			...
			catch //(Exception ex)
			{
				//if (m_debuggingNotifications == true)
				//{
				//	Program.Display("Error occurred on receiving feed back. - " + ex.Message);
				//}
